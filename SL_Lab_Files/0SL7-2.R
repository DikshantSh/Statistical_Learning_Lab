...
# Get min/max values of age using range() function
agelims = Wage %>%
  select(age) %>%
  range
agelims
...
ggplot() + 
  ...
  geom_line(aes(x = fit_smooth_cv$x, y = fit_smooth_cv$y), color = '6.8 effective degrees of freedom') + 
...
# s() function used to indicate we like to use a smoothing splines
par(mfrow = c(1,3))
